...
[AddComponentMenu("Dialogue/Dialogue/Utility/Text Sprite")]
public class TextSprite : Text
{
    private Image icon;
    ...
    public void RepopulateImages()
    {
        ...
        foreach (DialogueSprite sprite in GetDialogue().spritesUsed)
        {
    ...
    public bool FitImagesOnText(int y)
    {
        ...
        int position = dialogue.spritesUsed[y].spritePosition + 16 + Mathf.CeilToInt(dialogue.spritesUsed[y].width / 2);
        ...
        icons[y].rectTransform.sizeDelta = new Vector2(_fontWidth * dialogue.spritesUsed[y].width, _fontHeight * dialogue.spritesUsed[y].height);
